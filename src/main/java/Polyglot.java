...
class Polyglot {
    ...
    public static void main(String[] args) {
        // Construim contextul pentru JS
        Context polyglot = Context.create();

        // Definim array-ul de cuvinte prin script JS
        ...
        // Map pentru grupare dupa CRC
        Map<Long, List<String>> grupuriCRC = new HashMap<>();

        System.out.println("--- Procesare si calcul CRC ---");

        // Procesam fiecare cuvant din array-ul polyglot
        for (int i = 0; i < array.getArraySize(); i++) {
            ...
            // HashMap
            grupuriCRC.computeIfAbsent(crc, k -> new ArrayList<>()).add(upper);

            System.out.printf("%-15s -> %d%n", upper, crc);
        }
        ...
        for (Map.Entry<Long, List<String>> entry : grupuriCRC.entrySet()) {
            // Daca lista are mai mult de un element, avem coliziune
